...
function nameOf(node) {
  ...
  return node.type === "MemberExpression" ? `${nameOf(node.object)}.${nameOf(node.property)}` : "";
}
...
function inventorySource(file, source) {
  ...
  return found.map(({ node, parent }, index) => {
    const name = functionName(node, parent, index);
    const parameters = node.params.map((p) => source.slice(p.start, p.end).replace(/\s+/g, " ").trim());
    const signature = `${name}(${parameters.join(", ")})`;
    const bodyText = source.slice(node.start, node.end).replace(/\s+/g, " ").trim();
    return {
      id: `${file}::${signature}`,
      logicalId: `${file}::${name}`,
      file,
      ...
      hash: crypto.createHash("sha256").update(bodyText).digest("hex").slice(0, 16),
    };
  });
}
...
function createInventory(root) {
  ...
  sourceFiles(root).forEach((file) => {
    try {
      files[file] = inventorySource(file, fs.readFileSync(path.join(root, file), "utf8"));
    } catch (error) {
...
function allFunctions(inventory) {
  return Object.values(inventory.files || {}).flat();
}
